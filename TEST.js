...
async function handleAPIRequest(request, env, path, method) {
  ...
  if (path.startsWith('/api/transactions/') && method === 'DELETE') {
    ...
    let transactions = await kv.get(`transactions_${userId}`, 'json') || [];
    const initialLen = transactions.length;
    transactions = transactions.filter(t => t.id !== transactionId);
    if (transactions.length !== initialLen) await kv.put(`transactions_${userId}`, JSON.stringify(transactions));
    return serveJSON(transactions);
  }
...
function getServiceWorker() {
  const CACHE_NAME = 'aurora-v28-theme-switch'; 
  return `
